...
// const CURRENT_USER_ID = "user-jordan";
// const DEFAULT_PROJECT_ID = "project-launch";
...
//   seedPromise = (async () => {
//     const [existingUser] = await db.select().from(usersTable).where(eq(usersTable.id, CURRENT_USER_ID));
//     if (existingUser) return;

//     await db.insert(usersTable).values({
//     id: CURRENT_USER_ID,
//     name: "Jordan Lee",
//     email: "jordan@zana.app",
//     initials: "JL",
//     });
...
//       name: "Product launch",
//       description: "Everything needed to take the next release from idea to shipped.",
//       color: "ink",
//     },
//     {
//       id: "project-studio",
//       name: "Studio refresh",
//       description: "A sharper home for the studio and the work behind it.",
//       color: "stone",
//     },
//     {
//       id: "project-writing",
//       name: "Writing practice",
//       description: "A small, steady system for publishing more often.",
//       color: "paper",
//     },
//     ]);
//     await db.insert(projectMembersTable).values([
//     { id: "member-jordan-launch", projectId: DEFAULT_PROJECT_ID, userId: CURRENT_USER_ID, email: "jordan@zana.app", name: "Jordan Lee", initials: "JL", role: "owner", status: "active" },
//     { id: "member-maya-launch", projectId: DEFAULT_PROJECT_ID, userId: "user-maya", email: "maya@zana.app", name: "Maya Chen", initials: "MC", role: "member", status: "active" },
//     { id: "member-omar-launch", projectId: DEFAULT_PROJECT_ID, userId: "user-omar", email: "omar@zana.app", name: "Omar Haddad", initials: "OH", role: "member", status: "active" },
//     { id: "member-jordan-studio", projectId: "project-studio", userId: CURRENT_USER_ID, email: "jordan@zana.app", name: "Jordan Lee", initials: "JL", role: "owner", status: "active" },
//     { id: "member-jordan-writing", projectId: "project-writing", userId: CURRENT_USER_ID, email: "jordan@zana.app", name: "Jordan Lee", initials: "JL", role: "owner", status: "active" },
//     ]);
//     await db.insert(tasksTable).values([
//     { id: "task-research", projectId: DEFAULT_PROJECT_ID, title: "Research onboarding friction", description: "Talk to three people who joined last month and capture the common thread.", status: "not_done", assigneeId: "user-maya", position: 0 },
//     { id: "task-brief", projectId: DEFAULT_PROJECT_ID, title: "Write the release brief", description: "Turn the scattered notes into one clear page the team can rally around.", status: "doing", assigneeId: CURRENT_USER_ID, position: 0 },
//     { id: "task-demo", projectId: DEFAULT_PROJECT_ID, title: "Record product walkthrough", description: "A concise walkthrough for the first announcement.", status: "doing", assigneeId: "user-omar", position: 1 },
//     { id: "task-pricing", projectId: DEFAULT_PROJECT_ID, title: "Confirm pricing page copy", description: "Final review with the words team.", status: "done", assigneeId: CURRENT_USER_ID, position: 0 },
//     { id: "task-qa", projectId: DEFAULT_PROJECT_ID, title: "Run final QA pass", description: "Check the critical path on desktop and mobile.", status: "done", assigneeId: "user-maya", position: 1 },
//     ]);
//     await db.insert(activityTable).values([
//     { id: "activity-1", text: "Maya moved Research onboarding friction to Not done", time: "12 min ago", kind: "task" },
//     { id: "activity-2", text: "Jordan created a new project, Writing practice", time: "2 hours ago", kind: "project" },
//     { id: "activity-3", text: "Omar joined Product launch", time: "Yesterday", kind: "member" },
//     ]);
//   })();
...
// async function addActivity(text: string, kind: "task" | "project" | "member") {
//   await db.insert(activityTable).values({ id: randomUUID(), text, time: "Just now", kind });
// }
...
//   const activities = await db.select().from(activityTable).orderBy(desc(activityTable.createdAt)).limit(6);
...
//     status: "active",
//   });
//   await addActivity(`Jordan created a new project, ${project.name}`, "project");
...
//   await addActivity(`Jordan created ${task.title}`, "task");
...
//   const member = { id: randomUUID(), projectId: params.data.projectId, userId: null, email, name, initials: initialsFor(name), role: "member", status: "invited" };
//   await db.insert(projectMembersTable).values(member);
//   await addActivity(`Jordan invited ${email}`, "member");
...
//   await db.insert(tasksTable).values(task);
//   await db.update(projectsTable).set({ updatedAt: timestamp }).where(eq(projectsTable.id, task.projectId));
//   await addActivity(`${membership.name} created ${task.title}`, "task");
//   res.status(201).json(CreateTaskResponse.parse(await taskView(task)));
// });
...
//   await db.insert(projectMembersTable).values(member);
//   await addActivity(`${membership.name} invited ${email}`, "member");
...
router.delete("/projects/:projectId/members/:memberId", async (req, res): Promise<void> => {
  ...
  await addActivity(activityMessage, "member");
